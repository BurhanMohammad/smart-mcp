...
def _resolve_repo_paths() -> list[str]:
    \"\"\"
    Resolve one or more repository paths dynamically.

    Priority:
      1. CLI arguments
      2. MCP / IDE environment variables
      3. REPO_PATH env variable
      4. Current working directory

    Supports multiple repositories:
      python server.py repo1 repo2 repo3
    \"\"\"
    ...
    for var in _CLIENT_ENV_VARS:
        ...
        if val and _is_valid_project_dir(val):
            ...
            if abs_path not in resolved:
                logging.info(f"Repo path from env var {var}: {abs_path}")
    ...
    if repo_env:
        for repo in repo_env.split(os.pathsep):
            repo = repo.strip()

            if repo and _is_valid_project_dir(repo):
                abs_path = os.path.abspath(repo)

                if abs_path not in resolved:
                    logging.info(f"Repo path from REPO_PATH env var: {abs_path}")
                    resolved.append(abs_path)
    ...
    if _is_valid_project_dir(cwd):
        ...
        if abs_path not in resolved:
            logging.info(
                f"Repo path auto-detected from working directory: {abs_path}"
            )
            resolved.append(abs_path)
    ...
    if not resolved:
        logging.warning(
            ...
            "  Options:\n"
            "    a) CLI args:  python server.py <repo1> <repo2>\n"
            "    b) Env var:   REPO_PATH=<repo1>;<repo2>\n"
...
def _auto_build_index() -> None:
    ...
    for repo_path in repo_paths:
        ...
        try:
            ...
            logging.info(f"Successfully indexed: {repo_path}")

        except Exception as exc:
            logging.exception(
                f"Failed to index repository: {repo_path} -> {exc}"
            )
